...
const TourRegistrationPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Note: Simple Google Apps Scripts cannot handle file uploads.
    // This is included for UIs that might connect to a more advanced backend.
    if (licensePhoto) {
    ...
    try {
      ...
      // Treat the typical "Failed to fetch" CORS error from Google Scripts as a success
  ...
  const inputClasses = "mt-1 w-full p-3 bg-white/5 border border-black/10 rounded-md text-warm-text placeholder:text-warm-text/50 focus:outline-none focus:ring-2 focus:ring-warm-gold-light";

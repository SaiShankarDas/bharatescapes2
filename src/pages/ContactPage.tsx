...
const ContactPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // This part will likely not be reached if the script redirects, which is the common case.
      // But it's good to have for scripts that are configured to return a proper JSON response.
      if (response.ok) {
        ...
        // Handle cases where the server returns an error status code without throwing.
        throw new Error(`Server responded with status: ${response.status}`);
      }
    } catch (error) {
      // The most common "error" with Google Apps Script is a TypeError due to a cross-origin redirect after a successful POST.
      // We check for this specific case and treat it as a success.
      if (error instanceof TypeError && error.message === 'Failed to fetch') {
        ...
        // This handles actual network errors or other unexpected issues.
  ...
  const inputClasses = "contact-form-input mt-1 w-full p-3 bg-white/10 border border-white/20 rounded-md text-text-light placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-accent-gold";
